function [y, count] = SOR(a_n, b_n, q_n, dy0_or_dyf, x0, xf, y0, yf, i_num, h, Tol, Omega)
% ------------------ Description ---------------- %
%
% Using SOR method to iteratively compose estimated 'y'
%
% ----------- Initialization (Ay = b) ----------- %
...
while (err > Tol && n < N)
    ...
    % ------ Solve y_N :: Thomas algorithm ------ %
    y_N_GS(2, :) = Thomas_Algorithm(Lp, b - U*y_N(1, :)');
    % -------------- SOR Solution --------------- %
    y_N(2, :) = y_N(1, :) + Omega*(y_N_GS(2, :) - y_N(1, :));
